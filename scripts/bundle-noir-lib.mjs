...
// Generate TypeScript content with template literals wrapped in encode()
const filesContent = Object.entries(files)
  .sort(([a], [b]) => a.localeCompare(b))
  .map(([path, content]) => {
    // Use forward slashes for consistency
    const normalizedPath = path.replace(/\\/g, '/');
    return `  ["/noir/lib/${normalizedPath}", encode(\`${escapeForTemplateLiteral(content)}\`)]`;
  })
  .join(',\n');
 ...
 * 
 * This module provides all the noir/lib sources as Uint8Array for
 * in-memory filesystem compilation without disk access.
 * 
 * ${externalDepsNote}
 */
...
/**
 * Map of absolute path -> Uint8Array file content
 * Paths are absolute (e.g., "/noir/lib/consts/Nargo.toml")
 */
const NOIR_LIB_FILES: Map<string, Uint8Array> = new Map([
${filesContent}
]);
