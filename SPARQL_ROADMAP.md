...
7. **Support for `xsd:decimal` arbitrary precision.** Strict SPARQL 1.1 says decimals are arbitrary precision. The `noir_xpath` IEEE 754 path does not preserve this; `arith::Float` (base 10) approximated it but is in the deletion path under the Q1 = (a) decision (2026-05-03). Worth flagging that "SPARQL 1.1 compliant decimals" is technically not on the roadmap — almost no real query relies on it, but it would block formal compliance claims. **Pending decision** — recommended answer "no, document the deviation in the paper": with `xsd:decimal` bound to fixed precision, the W3C tests that exercise true arbitrary precision become known-skipped; list them in a paper appendix.
