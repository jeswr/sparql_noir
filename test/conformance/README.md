...
# Default: transform-only mode, all ~270 evaluation tests, JSON report.
...
# Full mode: also drives prove + verify via ts.js. Slow.
# Requires `npm run build` to have been run first.
npx tsx test/conformance/run-w3c-sparql10.ts --full --limit=5
```
...
The manifest + per-test data is cached under `temp/manifest-cache/`,
shared with `ts.js`'s cache. `--no-cache-manifest` forces a re-fetch.
...
3. **Lowers** the query through the WASM-compiled `transform/` module.
   The default mode stops here and records `passed` if lowering
   succeeds; this is the §8.2 "supported algebra fragment" rate.
4. **(--full only)** Compiles the resulting Noir circuit, signs the
   data, generates the proof via the `dist/scripts/prove.js` pipeline,
   and verifies — i.e. the legacy `ts.js` end-to-end flow, scoped to
   one test. The `passed` outcome here is the stronger "we close the
   prove/verify loop" rate.
...
| Status | Meaning |
|---|---|
| `passed` | Harness exercised the test successfully (transform OK in default mode; transform + prove + verify OK in `--full`). |
| `failed` | Harness exercised the test and the result was wrong (or verify rejected, in `--full`). |
...
`workflow_dispatch` allows running the slower `--full` mode on demand.
