#!/usr/bin/env node
/**
 * W3C SPARQL 1.0 conformance harness for sparql_noir.
 *
 ...
 *  3. (Optional, with --full) compile + prove + verify, and compare the
 *     result against the expected bindings / boolean.
 *  4. Record an outcome row: passed / failed / unsupported / errored / skipped.
 *
 ...
 *
 * Usage:
 *   npx tsx test/conformance/run-w3c-sparql10.ts                 # transform-only (default)
 *   npx tsx test/conformance/run-w3c-sparql10.ts --full          # also prove/verify
 ...
 *
 * CI-friendly defaults: transform-only, threshold=0 (never gate), JSON
 * report uploaded as artefact for paper §8.2.
 */
...
function parseCli(argv: string[]): CliOptions {
  ...
  if (has('--help') || has('-h')) {
    ...
    console.log(`
W3C SPARQL 1.0 conformance harness

Options:
  --full                  Run prove + verify in addition to transform (slower).
...
`);
  ...
  return {
    mode: has('--full') ? 'full' : 'transform-only',
...
// ---------------------------------------------------------------------------
// Manifest loading — reuses the same caching layout as ts.js so a local
// dev run shares the cache with the legacy runner.
// ---------------------------------------------------------------------------
...
const manifestCacheDir = path.join(sparqlNoirRoot, 'temp', 'manifest-cache');
  ...
  return allEntries.map((test) => {
    ...
    return {
      ...
      queryResult: t.queryResult
        ? { type: t.queryResult.type, value: t.queryResult.value }
        : null,
    };
  });
}
...
async function runOneTest(
  ...
  // 4. Full mode: compile, prove, verify, compare.
  // Reuses ts.js's ground-truth pipeline via dynamic import. Heavy and
  // gated behind --full; CI runs transform-only.
  return await runFullPipeline(candidate, parsed, baseRecord, startedAt);
}

interface FullPipelineParts {
  signRdfData?: (file: string) => Promise<unknown>;
  processRdfDataWithoutSigning?: (file: string) => Promise<unknown>;
  generateProofs?: (args: Record<string, unknown>) => Promise<unknown>;
  verifyProofs?: (args: Record<string, unknown>) => Promise<{ success: boolean }>;
}

let pipelineParts: FullPipelineParts | null = null;
let pipelineLoadError: string | null = null;

async function loadPipeline(): Promise<FullPipelineParts | null> {
  if (pipelineParts) return pipelineParts;
  if (pipelineLoadError) return null;
  try {
    const sign = await import('../../dist/scripts/sign.js' as string);
    const prove = await import('../../dist/scripts/prove.js' as string);
    const verify = await import('../../dist/scripts/verify.js' as string);
    pipelineParts = {
      signRdfData: (sign as { signRdfData?: FullPipelineParts['signRdfData'] })
        .signRdfData,
      processRdfDataWithoutSigning: (
        sign as {
          processRdfDataWithoutSigning?: FullPipelineParts['processRdfDataWithoutSigning'];
        }
      ).processRdfDataWithoutSigning,
      generateProofs: (
        prove as { generateProofs?: FullPipelineParts['generateProofs'] }
      ).generateProofs,
      verifyProofs: (
        verify as { verifyProofs?: FullPipelineParts['verifyProofs'] }
      ).verifyProofs,
    };
    return pipelineParts;
  } catch (err) {
    pipelineLoadError = err instanceof Error ? err.message : String(err);
    return null;
  }
}
...
async function runFullPipeline(
  candidate: CandidateTest,
  _parsed: { sparql_nr?: string; main_nr?: string },
  baseRecord: TestRecord,
  startedAt: number,
): Promise<TestRecord> {
  // The full prove/verify pipeline is identical to what ts.js drives — we
  // intentionally do NOT duplicate that logic here. If --full is requested
  // and dist/ is built, we delegate to ts.js per-test via the existing
  // CLI; otherwise we report a clean "pipeline-unavailable" outcome.
  ...
  if (!pipeline) {
    return {
      ...
      reason:
        pipelineLoadError ??
        'Full pipeline unavailable — run `npm run build` then re-run with --full',
      stage: 'compile',
      durationMs: Date.now() - startedAt,
    };
  }

  // Delegate to ts.js for the heavy lifting. ts.js writes a temp dir
  // per-test and exits 0 on success, non-zero on failure. We invoke it
  // with --filter pinned to the test name (ts.js supports regex filter).
  const escapedName = candidate.test.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  try {
    const tsJsPath = path.join(sparqlNoirRoot, 'ts.js');
    execSync(
      `node "${tsJsPath}" --filter="^${escapedName}$" --concurrency=1 --no-N`,
      { cwd: sparqlNoirRoot, stdio: 'pipe', timeout: 5 * 60_000 },
    );
    return {
      ...baseRecord,
      status: 'passed',
    ...
    return {
      ...baseRecord,
      status: 'failed',
      reason: `ts.js full pipeline rejected the test: ${message
        .split('\n')
        .slice(-3)
        .join(' | ')}`,
      stage: 'verify',
...
async function runWithConcurrency(
  ...
  async function worker(): Promise<void> {
    while (nextIndex < total) {
      ...
      const result = await runOneTest(candidate, mode, transform);
      results[idx] = result;
      completed++;
      const symbol =
...
async function main(): Promise<void> {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `Loading manifest: ${opts.manifestUrl} (mode=${opts.mode}, concurrency=${opts.concurrency})`,
  );
