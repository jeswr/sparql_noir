...
```noir
struct TermWitness {
    hash:   Field,                  // term hash (Enc_t result, sec.3)
    bytes:  [u8; STRING_LEN_MAX],   // bounded lexical bytes (advisory; see sec.6)
...
The Merkle commitment is computed over `[terms[0].hash, ..., terms[3].hash]` exactly as in sec.4 -- `bytes` and `length` are *not* part of the hash chain. See sec.6 for the witness's role in string operations.
...
Hash-only term encoding (sec.3) supports equality (BGP matching, FILTER `=`) but rules out string operators that need raw lexical content -- `STRSTARTS`, `STRENDS`, `CONTAINS`, `STRLEN`, `LANG`, `REGEX`, `IRI(...)`, `STRDT(...)`, `SHA256(...)`. SPARQL_ROADMAP.md sec.6.3 / sec.8.Q3 records the decision: extend the term witness with a bounded byte buffer plus a length witness.
...
`STRING_LEN_MAX` is a public global in `noir/lib/consts` (default `64`), configurable via `setup.ts` exactly like `MERKLE_DEPTH`, `stringHash`, etc. (see sec.6.5).
...
- Instead, the binding is **caller-supplied**. Round-2 string-operator lowering emits, immediately before any `bytes`-touching code, a constraint of the form
...
`encode_string_bounded` MUST round-trip with `encode_string` for any honest prover: `encode_string_bounded(bytes, length) == encode_string(s)` whenever `bytes[0..length]` is the UTF-8 encoding of `s`. The current implementation routes through a variable-length string-hash primitive `hash_string_var(buf, len)` (template-substituted to `sha256_var` under the default sha256 config); other configured string hashes (blake2s / blake3 / keccak256 / sha384 / sha512) lack a variable-length Noir API and assert under those configs so the round-2 byte-binding contract remains sound.

This contract is documented at the top of `noir/lib/types/src/lib.nr`.

### 6.4 Cost

Per triple: `4 x STRING_LEN_MAX` bytes of private input (4 x 64 = 256 bytes by default), plus `4 x u32` for the length witnesses. No extra constraints in round 1; round 2 adds one `hash_string`-equality constraint per byte-using term per use site.

For `STRING_LEN_MAX = 64` and a 4-triple BGP, the witness blob grows by `4 x 4 x 64 = 1024` bytes -- negligible against signature / Merkle path data. Picking `STRING_LEN_MAX = 256` quadruples the byte footprint and quadruples the (round-2) `hash_string` cost. Picking `STRING_LEN_MAX = 32` halves both but truncates lexical values longer than 32 bytes, breaking string operations on those terms (the prover cannot supply a witness that satisfies the length bound, so the proof fails to construct).
...
- The encode binary (`noir/bin/encode`) and the `sign.ts` / `prove.ts` data flow now produce the bytes / length witness alongside each term hash; non-literal positions (NamedNode / BlankNode) supply the IRI / blank-id bytes verbatim, since those forms are also lexical strings under sec.3.
