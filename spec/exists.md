...
- **Sentinels are mandatory.** Round-3 main event originally shipped a "no implicit sentinels" contract on the (incorrect) reasoning that `consts::hash4`'s collision-resistance made boundary-falling `absent_hash` values "vanishingly rare". Copilot was right on `sparql_noir#42` (2026-05-03): the collision-resistance argument is irrelevant — what matters is the order-statistic distribution. For `n` uniformly-distributed real leaves and a uniform `absent_hash`, `E[X_(1)] = F/(n+1)`, `E[X_(n)] = nF/(n+1)`, so `P(absent ∉ [X_(1), X_(n)]) = 2/(n+1)` — ~18% at n=10, ~2% at n=100, far above "vanishing". Round-3 follow-up (PR `non-membership-sentinels`) lands the fix:
  ...
  - A real triple whose `consts::hash4(triple)` equals a sentinel value (`0` or `p - 1`) is a **preimage event**, not a collision: the prover must produce an input whose hash hits one of two specific outputs. Under Pedersen-hash preimage-resistance the probability of any specific output is `≤ 1/p ≈ 2^-254` (BN254's order), so this is negligible rather than impossible. (Earlier wording on `sparql_noir#42` and `#47` invoked "collision-resistance"; that property concerns finding `x ≠ y` with `H(x) = H(y)`, which is a different argument.) A real-world preimage finding would itself be a publishable cryptographic event, in which case the sentinel constants would rotate.
...
- **Transform-side wiring (shipped — `non-membership-sentinels-transform-wiring`).** Each `NonExistenceConstraint` lowers to a runtime-dispatched call gated on a public `boundary_cases[i]: Field` input. The dispatch:
  - `0` (Lower) → `utils::verify_non_membership_low_sentinel_no_inclusion(low_sentinel, bgp[right], absent_hash)`,
  - `1` (Middle) → `utils::verify_non_membership_no_inclusion(bgp[left], bgp[right], absent_hash)`,
  - `2` (Upper) → `utils::verify_non_membership_high_sentinel_no_inclusion(bgp[left], high_sentinel, absent_hash)`,
  - any other tag → `assert(false, ...)`.

  The generated `main.nr` accepts two `SentinelLeaf` inputs (`low_sentinel`, `high_sentinel`) and a `boundary_cases: pub BoundaryCases` array (one tag per constraint), runs `verify_low_sentinel_inclusion` / `verify_high_sentinel_inclusion` against `roots[0].value` once each, then threads everything into `checkBinding`. The verifier sees `boundary_cases` as a public input so a malicious prover cannot silently switch which dispatch arm fires; the chosen primitive's strict-`<` / adjacency assertions enforce non-membership against the genuine bracket pair (a prover lying about the case fails the assertions of the chosen arm).

  In Lower mode `bgp[bracket_left_idx]` is a prover-supplied dummy (still inclusion-checked by the generic per-triple loop, but its hash isn't used by the dispatch); same for `bgp[bracket_right_idx]` in Upper. The TS prover-side witness generator picks the tag at proof time by comparing the live `absent_hash` against the dataset's smallest / largest real leaf hashes.

  Snapshots: `transform/tests/snapshots/not_exists_grounded.{sparql,main}.nr`, `not_exists_outer_var.*`, `minus_basic.*`. Noir-level dispatch tests: `test_transform_dispatch_lower_boundary` / `_middle_interior` / `_upper_boundary` / `_invalid_boundary_case_rejected` in `noir/lib/utils/src/lib.nr`.
- **Tree capacity.** The mandatory sentinels shift the level-0 capacity requirement from `N` to `N + 2`; `merkle()` enforces `N + 2 <= 1024` via `std::static_assert` so a too-large `N` fails at circuit-compile time rather than overwriting padding. With the standard `MERKLE_DEPTH = 11`, the maximum real-leaf count is therefore `1022` (down from `1024`). Roborev finding 2026-05-03 (medium).
...
- **NOT EXISTS** lowering: **single-triple ground-inner only** — every position in the inner triple is either constant or a variable already bound by the outer μ. Lowers to a `NonExistenceConstraint`: two bracket-leaf BGP slots are appended (auto-inclusion-checked); the constraint emits a runtime-dispatched call to one of the `verify_non_membership_*_no_inclusion` family gated on the public per-constraint `boundary_cases[i]` Field — Lower / Middle / Upper, with sentinel-leaf inputs threaded from `main.nr` (see §3.3). Multi-triple inner / non-ground inner / nested NOT-EXISTS / inner UNION-or-OPTIONAL are rejected with clear errors — see §7.
...
- The emit layer wraps each `NonExistenceConstraint` in an `if boundary_cases[i] == X` dispatch chain over the three `verify_non_membership_*_no_inclusion` primitives, with `assert(false)` on out-of-range tags. `main.nr` runs `verify_low_sentinel_inclusion` / `verify_high_sentinel_inclusion` once each before threading the `SentinelLeaf` arguments into `checkBinding`. Easy-case OPTIONALs additionally emit `assert(matched | unmatched)` lines using the boolean variant `verify_non_membership_no_inclusion_check`.
