...
- **Transform-side wiring -- explicitly incomplete in this PR.** `transform/src/lower.rs::lower_not_exists_into` and `transform/src/emit.rs` currently emit only the interior-case primitive call (`utils::verify_non_membership_no_inclusion(bgp[k], bgp[k+1], absent_hash)`); the generated `bgp` only carries real `Triple` inputs, not `SentinelLeaf` inputs. As a result, **end-to-end `NOT EXISTS` proofs still cannot witness boundary-case absent hashes** -- the underlying primitives in `noir/lib/utils` are ready, but the lowering / `main.nr` template / prover-side I/O are not yet plumbed through. Concrete next steps:
   1. Extend the generated `main.nr` to accept two `SentinelLeaf` inputs (`low_sentinel`, `high_sentinel`) and call `verify_low_sentinel_inclusion` / `verify_high_sentinel_inclusion` against the signed root.
   2. Add a per-`NonExistenceConstraint` discriminator (low / interior / high) into `crate::ir`, populated at prove time once the absent hash is evaluated against the witnessed bracket leaves.
   3. Update `emit.rs` to emit the appropriate sentinel-aware call.
   4. Update the TS prover to route the bracket-selection logic.
   Roborev finding 2026-05-03 (medium) -- explicitly tracked here so the feature is not advertised as "complete" prematurely.
